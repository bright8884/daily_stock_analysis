...
> 本节为 `Refs #1497` 的文档边界澄清（仅文档说明，不构成运行时交付），不包含 Actions 触发机制改造；本次改动属于文档类型（`docs + Refs #1497`，即 PR Type 仅勾选 `docs` 且 Issue Link 使用 `Refs #1497`），验收范围限定为边界说明一致性、文档双语同步、changelog 同步与 PR 模板提示对齐，不覆盖运行时触发控制实现，不应作为 `#1497` 的 issue 关闭依据。
> PR 元信息必须以 `docs + Refs #1497`（即 PR Type 标记为 `docs`，Issue Link 使用 `Refs #1497`，而非 `Closes #1497`）统一写入，以免误关闭仍待实现的运行时能力。
...
该说明对应 `Refs #1497` 的文档边界澄清，仅限阐明当前行为，不代表已交付 Actions runtime 动态触发能力。
> 该说明为 `Refs #1497` 的文档边界澄清，动态触发能力需通过后续 PR 补齐，当前仅同步文档说明，不应视为运行时交付或 PR Close 条件。
