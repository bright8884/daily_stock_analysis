...
- [文档] `Refs #1497` 的边界说明（仅文档说明，不是运行时交付）：本次仅更新文档，不包含运行时改造；`SCHEDULE_TIME` 仍仅影响本地内置调度（`python main.py --schedule`），`GitHub Actions` 的触发时点仍由 `00-daily-analysis.yml` 的固定 `cron` 决定（周一到周五 10:00 UTC / 北京18:00）；本项属于 `docs` 范围，验收范围限定为边界说明一致性、文档双语同步、changelog 同步与 PR 模板提示对齐，PR 元信息应以 `docs + Refs #1497` 的形式归类（PR Type 仅勾选 `docs`，Issue Link 使用 `Refs #1497`），`#1497` 的运行时触发能力仍需后续 PR 实现，不应将本变更作为 issue 关闭依据。
