...
> Compatibility boundary for this PR: this market-review update only adds A-share hot-stock and limit-up ladder report blocks, does not introduce any new config keys, and does not perform migration/cleanup for existing model/provider settings.
...
> - Temporary rollback for this feature: set `MARKET_REVIEW_ENABLED=false` to bypass the enhancement; this does not mutate persisted model/provider configuration.
